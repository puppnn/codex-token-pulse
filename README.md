...
悬浮窗采用深色玻璃质感：状态色辉光聚焦当前账号，细分隔线保持信息密度，数字与中文标签使用不同字阶，便于在 390×760 的常驻窗口中快速扫读。以下截图使用脱敏演示数据生成。

### 账号

显示带状态辉光的当前账号、最近请求、今日统计、Token 趋势和账号用量排行。支持 `今日 / 5h / 7d / 30d` 切换。

![账号页](assets/screenshots/accounts.png)

### 用量统计

提供 `今日 / 7d / 30d / 全部` 视图，包含重新排布的 Token 汇总、构成、缓存命中率、活跃分布、常用模型和账号累计成本。
